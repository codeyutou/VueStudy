<template>
  ...
  <div class="outer">
    ...
    <TypeNav />
    我是搜索
    <h2>param参数{{ $route.params.keyword }}</h2>
    <h2>query参数{{ $route.query.category1id }}</h2>
    <!--list-content-->
    <div class="main">
...
export default {
  ...
  beforeMount() {
    ...
    // const {categoryName,category3Id,category2Id,category1Id}=this.$route.query
  ...
  methods: {
    getSearch(searchParams) {
      this.$store.dispatch("search/getSearchList", searchParams);
    },
    removeCategory() {
  ...
  watch: {
    $route(newValue, oldValue) {
      Object.assign(this.searchParams, this.$route.query, this.$route.params);
      console.log("检测到路由变化", this.$route.query, this.$route.params);
      this.getSearch(this.searchParams);
